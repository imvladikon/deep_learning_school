...
if __name__ == '__main__':
    ...
    weight = Path("./weights") / "pretrained.pt"
    examples_folder = Path("./examples")

    style_model = Net(ngf=128)
    style_model.load_state_dict(torch.load(weight), False)

    content_image = tensor_load_rgbimage(examples_folder/'venice-boat.jpg', size=512,
                                         keep_asp=True).unsqueeze(0)
    style = tensor_load_rgbimage(examples_folder/'candy.jpg', size=512).unsqueeze(0)
    ...
    tensor_save_bgrimage(output.data[0], examples_folder/'output.jpg', False)